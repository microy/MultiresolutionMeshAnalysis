/***************************************************************************
                             ...
                             -------------------
    update               : 2009/02/19
    copyright            : (C) 2002-2009 by Michaël Roy
...
class VectorT
{
	public :
		...
		inline VectorT(const VectorT<Type,Size>& v) {
			std::memcpy(values, v.values, Size*sizeof(Type));
		}

		explicit inline VectorT(const Type val[Size]) {
			std::memcpy(values, val, Size*sizeof(Type));
		}
		...
		inline VectorT<Type,Size>& Clamp(const Type& min, const Type& max) {
			assert( min < max );
