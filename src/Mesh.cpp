...
//--
//
// ConvertTexture2Colors
//
//--
// Convert texture image to mesh colors
bool Mesh::ConvertTexture2Colors()
{
	// Check texture presence
	if( TextureNumber()==0 || TextureName().empty()==true ) return false;

	// Read texture image
	TextureImage texture;
	texture.ReadFile( TextureName() );

	// Get image information
	int w = texture.Width();
	int h = texture.Height();

	// Convert Texture image to mesh color
	Colors().resize( VertexNumber() );
	for( int i=0; i<VertexNumber(); i++ ) {
		const double& u = Texture(i)[0];
		const double& v = Texture(i)[1];
		double r = texture.Buffer((int)(u*((double)w-1.0))*3+(int)(v*((double)h-1.0))*w*3);
		double g = texture.Buffer((int)(u*((double)w-1.0))*3+(int)(v*((double)h-1.0))*w*3+1);
		double b = texture.Buffer((int)(u*((double)w-1.0))*3+(int)(v*((double)h-1.0))*w*3+2);
		Color(i) = Vector3d( r/255.0, g/255.0, b/255.0 );
	}
...
void Mesh::Scale(const double& scale)
{
	assert( scale != 0 );
	for( int i=0; i<VertexNumber(); i++ ) {
		Vertex(i) *= scale;
